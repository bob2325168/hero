...
import (
	...
	framework2 "github.com/bob2325168/gohero/framework"
...
var providerListCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		heroContainer := container.(*framework2.HeroContainer)
		// 获取字符串凭证
		list := heroContainer.NameList()
		// 打印
...
var providerCreateCommand = &cobra.Command{
	...
	RunE: func(c *cobra.Command, args []string) error {
		...
		// 检查服务是否存在
		providers := container.(*framework2.HeroContainer).NameList()
